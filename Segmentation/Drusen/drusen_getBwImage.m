...
%--------------------------------------------------------------------------
%
%  function [bwImage, borders] = drusen_getBwImage( ...
...
function [bwImage, borders] = drusen_getBwImage( ...
    ...
    % Sort the lines in ascending order. Go column by column since some
    % lines may have intersected with each other
    borders = NaN(params.NUM_BORDERS,imageWidth);
    oddIndices = 1:2:params.NUM_BORDERS;
    evenIndices = 2:2:params.NUM_BORDERS;
    for iCol = 1:imageWidth
        borders(oddIndices,iCol) = sort(lines(evenIndices,iCol));
        borders(evenIndices,iCol) = sort(lines(oddIndices,iCol));
    end
