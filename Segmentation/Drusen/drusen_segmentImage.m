%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  drusen_segmentImage.m
%
%  Segments retinal layers on an OCT image
%
%--------------------------------------------------------------------------
%
%  function layers = drusen_segmentImage(image, params)
%
%  INPUT PARAMETERS:
%
...
%                 set by the drusen_getParameters function
%
%  OUTPUT VARIABLES:
%
...
function layers = drusen_segmentImage(image, params)
    ...
    [bwImage, borders] = drusen_getBwImage( ...
    ...
    %----------------------------------------------------------------------
    %  Segment the ILM on the unflattened image
    %----------------------------------------------------------------------
    ...
    %----------------------------------------------------------------------
    % Flatten the image based on the ILM and bruchs
    %----------------------------------------------------------------------
        
    rpe = smooth((rpeTop + rpeBottom) / 2)';
    [image,pixelShift,invalidIndices] = drusen_flattenImage(image, lateralRes, ilm, rpe);
    ...
    %----------------------------------------------------------------------
    %  Segment layers using the image flattened by bruchs
    %----------------------------------------------------------------------

    layers = drusen_graphCut( ...
        image, ...
        axialRes, ...
        lateralRes, ...
        rpeTop, ...
