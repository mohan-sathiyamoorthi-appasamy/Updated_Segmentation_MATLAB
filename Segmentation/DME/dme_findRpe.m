...
%--------------------------------------------------------------------------
%
%  function rpe = dme_findRpe(image, axialRes, lateralRes, rpe, bruchs)
%
%  INPUT PARAMETERS:
%
...
function rpe = dme_findRpe(image, axialRes, lateralRes, rpe, bruchs)
    ...
    topLayer = rpe;
    bottomLayer = bruchs;
    indices = (bottomLayer - topLayer > round(45.5/axialRes));
    bottomLayer(indices) = max(topLayer(indices),bottomLayer(indices)-round(15/axialRes));
    indices = (bottomLayer - topLayer > round(100/axialRes));
    bottomLayer(indices) = max(topLayer(indices),bottomLayer(indices)-round(26/axialRes));
    ...
    for col = startCol:endCol
        ...
        y = find(smoothColumn == max(smoothColumn), 1);
        if y == 1
            rpe(col) = rpe(col);
        else
            rpe(col) = y;
