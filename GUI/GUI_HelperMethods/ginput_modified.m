function [out1,out2,out3] = ginput_modified(arg1,showClickedPoints,markerSize,color)
...
if nargin < 2 || isempty(showClickedPoints)
    showClickedPoints = 1;
end
...
%         clicked figure has handlevisibility set to callback
      if(ptr_fig == fig)
         ...
         if showClickedPoints
             colorStyle = strcat('.',color);
             figure(fig);
             hold on;
             if isempty(markerSize) || isnan(markerSize);
                plot(out1, y, colorStyle);
             else
                plot(out1, y, colorStyle, 'MarkerSize', markerSize);
             end
         end
      end
   end
